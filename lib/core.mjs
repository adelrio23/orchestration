...
export function providerOperationalState(name, provider, timestamp = Date.now()) {
  if (!provider.enabled) return { name, state: 'disabled', usable: false, automatic: false, message: 'Disabled by you.' };
  const blocked = provider.blocked || '';
  if (/^quota/.test(blocked)) {
    const readyAt = provider.availableAt || null;
    const waiting = readyAt && readyAt > timestamp;
    return { name, state: 'quota', usable: false, automatic: true, availableAt: readyAt, message: waiting ? `Usage exhausted. Automatic recovery is scheduled for ${new Date(readyAt).toISOString()}.` : 'Usage exhausted. An automatic recovery check is due.' };
  }
  if (/^auth/.test(blocked)) return { name, state: 'authentication', usable: false, automatic: false, message: 'Sign in to this CLI again, then re-enable the provider.' };
  if (blocked) return { name, state: 'blocked', usable: false, automatic: false, message: blocked };
  return { name, state: 'available', usable: true, automatic: true, message: 'Available for dispatch; live access is confirmed on use.' };
}
...
export function teamOperationalStatus(state, runningCount = 0, timestamp = Date.now()) {
  ...
  const waitingWork = state.tasks.some(task => ['queued', 'waiting'].includes(task.status));
  const result = { state: 'idle', headline: 'Team is ready', detail: 'No work is waiting.', providers, nextRecoveryAt: null };
  if (state.recoveryRequired) return { ...result, state: 'action_required', headline: 'Your attention is required', detail: state.recoveryRequired };
  if (state.calls >= state.limits.maxCalls) return { ...result, state: 'budget_exhausted', headline: 'Coordinator call budget exhausted', detail: `${state.calls} of ${state.limits.maxCalls} CLI attempts used. Raise the limit while paused to continue.` };
  if (runningCount) return { ...result, state: 'working', headline: `${runningCount} agent call${runningCount === 1 ? '' : 's'} working`, detail: 'Active calls remain bounded by their timeout and output limits.' };
  if (state.mode === 'paused') return { ...result, state: 'paused', headline: 'Coordinator paused', detail: waitingWork ? 'Work is queued and will not dispatch until you resume.' : 'No new work will dispatch.' };
  if (!waitingWork) return result;
  const usable = providers.filter(provider => provider.usable);
  if (usable.length >= 2) return { ...result, state: 'ready', headline: 'Team ready to work', detail: `${usable.length} independent providers are available.` };
  const recoveries = providers.filter(provider => provider.state === 'quota' && provider.availableAt).map(provider => provider.availableAt);
  const nextRecoveryAt = recoveries.length ? Math.min(...recoveries) : null;
  if (usable.length === 1) return { ...result, state: 'degraded', headline: 'Waiting for an independent agent', detail: nextRecoveryAt ? `One provider is usable. Another is expected after ${new Date(nextRecoveryAt).toISOString()}.` : 'One provider is usable, but building and independent review require two.', nextRecoveryAt };
  const auth = providers.filter(provider => provider.state === 'authentication').map(provider => provider.name);
  if (auth.length) return { ...result, state: 'action_required', headline: 'Agent login required', detail: `Sign in again for: ${auth.join(', ')}.`, nextRecoveryAt };
  if (nextRecoveryAt) return { ...result, state: 'waiting_for_usage', headline: 'Waiting for usage to reset', detail: `No provider can work now. Automatic recovery is scheduled for ${new Date(nextRecoveryAt).toISOString()}.`, nextRecoveryAt };
  return { ...result, state: 'blocked', headline: 'No agent can work', detail: 'Enable or repair at least two eligible providers.' };
}
    ...
    if (result.status !== 'ok') {
      ...
      if (['quota', 'auth'].includes(result.status)) {
        ...
        if (result.status === 'quota') {
          state.nextProbe = Date.now() + 1800000;
          state.availableAt ||= state.nextProbe;
          state.blocked = 'quota: usage exhausted; automatic recovery scheduled';
        } else {
          state.nextProbe = null; state.availableAt = null;
          state.blocked = 'auth: sign in to this CLI again, then re-enable it';
        }
        t.status = 'queued';
