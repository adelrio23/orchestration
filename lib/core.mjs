...
export function providerOperationalState(name, provider, timestamp = Date.now()) {
  ...
  if (/^quota/.test(blocked)) {
    ...
    return { ...base, state: 'quota', usable: false, automatic: true, availableAt: readyAt, message: waiting ? `Usage exhausted. Automatic recovery is scheduled for ${new Date(readyAt).toISOString()}.` : 'Usage exhausted. An automatic recovery check is due.' };
  }
