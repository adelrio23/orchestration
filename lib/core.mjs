...
export class Coordinator {
  ...
  async chat(body) {
    ...
    try {
      ...
      if (record.autoStart && record.plan?.status === 'draft') {
        ...
        this.state.runs.push({ ...review, id: crypto.randomUUID(), role: 'plan_review', provider: reviewer, at: now(), estimatedPromptTokens: Math.ceil(reviewPrompt.length / 4), estimateLabel: 'Character heuristic; excludes CLI system context and tool calls' });
  ...
  acceptPlan(id) {
    if (this.state.mode !== 'paused' || this.running.size || this.chatting || this.integrating || this.githubBusy || this.state.recoveryRequired) throw Error('Pause and drain active work before queuing a plan');
