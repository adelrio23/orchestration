...
export const defaults = { maxCalls: 60, maxAttempts: 2, concurrency: 1, maxTasks: 100, timeoutMs: 300000, testTimeoutMs: 60000, maxOutputBytes: 2000000, maxContextChars: 40000, maxFiles: 40, maxRepairRounds: 2, maxMilestones: 8 };
...
export class Coordinator {
  ...
  setLimits(body) {
    ...
    const ranges = { maxCalls: [1, 1000], maxAttempts: [1, 6], concurrency: [1, 4], maxTasks: [1, 500], timeoutMs: [1000, 1800000], testTimeoutMs: [1000, 900000], maxOutputBytes: [10000, 5000000], maxContextChars: [6000, 200000], maxFiles: [1, 60], maxRepairRounds: [0, 6], maxMilestones: [1, 12] };
  ...
  async monitorProviders(force = false, reader = codexRead) {
    ...
    try {
      ...
      if (relevant.some(w=>w.remaining===0)) { provider.blocked='quota: measured Codex limit exhausted'; }
      else if (relevant.length && provider.enabled && /^quota/.test(provider.blocked||'')) { provider.blocked=null;for(const t of this.state.tasks)if(t.status==='waiting'){t.status='queued';t.blocked=null}this.event('quota_recovered',{provider:'codex'}); }
  ...
  async recoverProvider() {
    ...
    if(!pair)return;
    const [name,p]=pair;this.monitoring=true;p.recoveryAttempts=(p.recoveryAttempts||0)+1;p.nextProbe=Date.now()+1800000*Math.pow(2,p.recoveryAttempts-1);this.state.calls++;
    ...
    try {
      ...
      if(result.status==='ok' && result.text.trim()==='READY') { p.blocked=null;p.recoveryAttempts=0;for(const t of this.state.tasks)if(t.status==='waiting'){t.status='queued';t.blocked=null}this.event('quota_recovered',{provider:name}); }
  ...
  prompt(t) {
    const context = {
      ...
      budget: { callsUsed: this.state.calls, callsRemaining: Math.max(0, this.state.limits.maxCalls - this.state.calls - 1), maxFilesPerAttempt: this.state.limits.maxFiles ?? defaults.maxFiles },
  ...
  async chat(body) {
    if (!this.state.repo) throw Error('Configure a repository first');
    if (this.state.mode !== 'paused' || this.running.size || this.integrating || this.chatting || this.monitoring || this.githubBusy || this.state.recoveryRequired) throw Error('Pause, recover interruptions and let active work finish before direct chat');
    ...
    const planning = intent === 'plan' ? `Act as the project lead. Inspect the existing repository as needed and propose only the next ${this.state.limits.maxMilestones ?? defaults.maxMilestones} or fewer small, sequential milestones, each implementable with at most ${this.state.limits.maxFiles ?? defaults.maxFiles} replacement files and independently testable by the configured test commands. Include tests in owned scope. Do not change commands or permissions. Do not repeat existing tasks. If information is essential, ask up to 3 focused questions and return no tasks. Return valid JSON inside <coordinator-plan>{"summary":"short explanation","questions":[],"tasks":[{"title":"milestone","requirements":"concrete behavior","acceptance":"verifiable criteria","scope":["src/","test/"]}]}</coordinator-plan>. No commands, providers, parallel flags or arbitrary dependencies in the plan. This is a draft: nothing will execute until the user queues it and starts work. Limits: ${JSON.stringify({ remainingCalls: this.state.limits.maxCalls-this.state.calls-1, testCommands: this.state.testCommands, existingTasks: this.state.tasks.map(t=>({title:t.title,status:t.status})) })}.` : 'Suggest a concise decision or task change if helpful.';
    ...
    if (record.autoStart && record.status === 'ok' && record.plan?.review?.passed) {
      try { this.acceptPlan(record.id); if ((this.state.pauseVersion || 0) === record.pauseVersion) { this.control('start'); record.phase = 'building'; } else record.phase = 'queued_and_paused'; }
  ...
  parsePlan(text, basis) {
    ...
    const tasks = raw.tasks.map(t => {
      const checked = this.makeTask({ title: t.title, requirements: t.requirements, acceptance: t.acceptance, scope: t.scope });
      return { title: checked.title, requirements: checked.requirements, acceptance: checked.acceptance, scope: checked.scope };
    });
    return { summary, questions, tasks, basis, status: questions.length ? 'questions' : 'draft' };
  }
  acceptPlan(id) {
    if (this.state.mode !== 'paused' || this.running.size || this.chatting || this.integrating || this.monitoring || this.githubBusy || this.state.recoveryRequired) throw Error('Pause and drain active work before queuing a plan');
