...
const FIX = {
  ...
  claude: windows()
    ? 'Claude runs through WSL here. Install it with: wsl -e bash -lc "npm install -g @anthropic-ai/claude-code", then run: wsl -e bash -lc claude and sign in with your Claude subscription.'
...
export async function agentReadiness(cwd, runner = run) {
  ...
  for (const [name, config] of Object.entries(adapters)) {
    const versionArgs = config.wsl ? ['--exec', config.exe || 'claude', '--version'] : ['--version'];
