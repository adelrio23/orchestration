...
// The coordinator only accepts a private target, so only private repositories
// are offered. Typing owner/repository by hand still works.
export async function privateRepositories(cwd, runner = run) {
  const spec = ghSpec(['repo', 'list', '--json', 'nameWithOwner,isPrivate', '--limit', '100'], cwd);
  const result = await runner(spec.command, spec.args, { cwd, timeout: 20000, env: safeEnv() });
  ...
  return list.filter(repo => repo.isPrivate && typeof repo.nameWithOwner === 'string').map(repo => repo.nameWithOwner).sort();
}
