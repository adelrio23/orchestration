...
export function invocation(name, config, prompt, role, cwd) {
  ...
  if (name === 'codex') return { command: config.command, args: ['exec', '--ignore-user-config', '--ephemeral', '--sandbox', 'read-only', '--json', '--color', 'never', '-'], input: prompt };
  if (name === 'kimi') return { command: config.command, args: ['--model', 'kimi-code/kimi-for-coding', '--agent-file', fileURLToPath(new URL('../profiles/kimi-reader.md', import.meta.url)), '-p', prompt, '--output-format', 'stream-json'] };
  if (name === 'claude') {
    const args = ['--print', '--output-format', 'json', '--permission-mode', 'plan', '--no-session-persistence', '--strict-mcp-config', '--tools', 'Read,Glob,Grep'];
