...
export function discover() {
  ...
  return {
    ...
    claude: { command: process.platform === 'win32' ? 'wsl.exe' : 'claude', wsl: process.platform === 'win32', exe: process.platform === 'win32' ? wslClaudePath() : 'claude' }
  };
}

// `wsl.exe --exec` runs the binary directly, with no shell, so it never sees the
// PATH a login shell would set — an npm -g install lands outside it and looks
// "not installed". Resolve the real path once through a login shell, then keep
// using --exec so no argument is ever shell-interpolated.
let resolvedClaude;
export function wslClaudePath() {
  if (resolvedClaude !== undefined) return resolvedClaude;
  try {
    const found = spawnSync('wsl.exe', ['-e', 'bash', '-lc', 'command -v claude'], { encoding: 'utf8', timeout: 15000, windowsHide: true });
    const path = found.status === 0 ? String(found.stdout).trim().split(/\r?\n/).filter(Boolean).pop() : '';
    resolvedClaude = /^\/[^\s]+$/.test(path || '') ? path : 'claude';
  } catch { resolvedClaude = 'claude'; }
  return resolvedClaude;
}
export function resetClaudePath() { resolvedClaude = undefined; }

export const CODEX_SANDBOXES = ['read-only', 'workspace-write', 'danger-full-access'];
export function codexSandbox(config) {
  return CODEX_SANDBOXES.includes(config?.sandbox) ? config.sandbox : 'read-only';
}

export function invocation(name, config, prompt, role, cwd) {
  const model = config.models?.[role] || config.model;
  const modelArgs = model ? ['--model', model] : [];
  const researching = role === 'research';
  // Codex enforces its sandbox with OS facilities that do not exist on Windows,
  // where it then refuses to run its own read commands and plans blind. The
  // sandbox is therefore configurable; 'read-only' stays the default.
  if (name === 'codex') return { command: config.command, args: ['exec', ...modelArgs, '--ignore-user-config', '-c', 'approval_policy="never"', ...(researching ? ['--search'] : []), '--ephemeral', '--sandbox', codexSandbox(config), '--json', '--color', 'never', '-'], input: prompt };
  if (name === 'kimi') return { command: config.command, args: ['--model', model || 'kimi-code/kimi-for-coding', '--agent-file', fileURLToPath(new URL(researching ? '../profiles/kimi-researcher.md' : '../profiles/kimi-reader.md', import.meta.url)), '-p', prompt, '--output-format', 'stream-json'] };
  if (name === 'claude') {
    // Research gets web tools and no file access; every other role gets the reverse.
    const tools = researching ? ['--tools', 'WebSearch,WebFetch', '--allowedTools', 'WebSearch,WebFetch'] : ['--tools', 'Read,Glob,Grep', '--allowedTools', 'Read(./**),Glob,Grep'];
    const args = [...modelArgs, '--print', '--output-format', 'json', '--permission-mode', 'plan', '--no-session-persistence', '--strict-mcp-config', ...tools];
    ...
    if (config.wsl) {
      ...
      return { command: config.command, args: ['--cd', unix, '--exec', 'env', '-u', 'ANTHROPIC_API_KEY', '-u', 'ANTHROPIC_AUTH_TOKEN', '-u', 'ANTHROPIC_BASE_URL', '-u', 'CLAUDE_CODE_USE_BEDROCK', '-u', 'CLAUDE_CODE_USE_VERTEX', '-u', 'CLAUDE_CODE_USE_FOUNDRY', 'timeout', '--kill-after=5s', '300s', config.exe || 'claude', ...args], input: prompt };
    }
...
export function parseResult(name, result) {
  ...
  else if (events.some(e=>Array.isArray(e.permission_denials)&&e.permission_denials.length)) status = 'permission';
  ...
  return { reportedModels, status, reason: result.reason || '', text: texts.join('\n').slice(-120000), usage, durationMs: result.durationMs, exitCode: result.code, diagnostic: status === 'ok' ? '' : result.output.slice(-3000) };
}

export async function preflight(name, config, cwd, runner = run) {
  let command = config.command, args;
  if (name === 'codex') args = ['login', 'status'];
  if (name === 'kimi') args = ['provider', 'list'];
  if (name === 'claude') args = config.wsl ? ['--cd', cwd.replaceAll('\\', '/').replace(/^([a-zA-Z]):/, (_, d) => `/mnt/${d.toLowerCase()}`), '--exec', config.exe || 'claude', 'auth', 'status'] : ['auth', 'status'];
  const r = await runner(command, args, { cwd, timeout: 15000, env: safeEnv() });
