...
export function invocation(name, config, prompt, role, cwd) {
  ...
  if (name === 'codex') return { command: config.command, args: ['exec', ...modelArgs, '--ignore-user-config', '-c', 'approval_policy="never"', '--ephemeral', '--sandbox', 'read-only', '--json', '--color', 'never', '-'], input: prompt };
  ...
  if (name === 'claude') {
    const args = [...modelArgs, '--print', '--output-format', 'json', '--permission-mode', 'plan', '--no-session-persistence', '--strict-mcp-config', '--tools', 'Read,Glob,Grep', '--allowedTools', 'Read(./**),Glob,Grep'];
