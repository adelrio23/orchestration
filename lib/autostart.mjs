...
export async function cloneFromGitHub(engine, body, deps = {}) {
  ...
  if (fs.existsSync(target)) throw Error(`${target} already exists; it may already have been cloned`);

  const spec = ghSpec(['repo', 'clone', body.name, toCloneTarget(target)], root);
