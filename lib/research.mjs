...
// --- Agent-led search, independently verified --------------------------------
// The agent searches the web itself and reports claims with URLs and quotes.
// A model can fabricate a plausible URL or a quote a page does not contain, so
// the coordinator re-fetches every cited page and checks the quote is really
// there. Verification is done by this code, not by the agent's say-so.

const normalise = text => String(text).toLowerCase().replace(/[‘’“”]/g, c => ({ '‘': "'", '’': "'", '“': '"', '”': '"' }[c])).replace(/\s+/g, ' ').trim();

export function parseFindings(text, maxFindings = RESEARCH_LIMITS.maxSources) {
  const matches = [...String(text).matchAll(/<coordinator-research>([\s\S]*?)<\/coordinator-research>/g)];
  if (matches.length !== 1) throw Error('The researcher must return exactly one <coordinator-research> block');
  const raw = JSON.parse(matches[0][1]);
  if (!Array.isArray(raw.findings) || !raw.findings.length) throw Error('The researcher returned no findings');
  if (raw.findings.length > maxFindings) throw Error(`At most ${maxFindings} findings per research round`);
  return raw.findings.map(finding => {
    const claim = String(finding.claim ?? '').trim(), url = String(finding.url ?? '').trim(), quote = String(finding.quote ?? '').trim();
    if (!claim || claim.length > 1000) throw Error('Each finding needs a claim of at most 1000 characters');
    if (!quote || quote.length > 1000) throw Error('Each finding needs a verbatim quote of at most 1000 characters');
    if (!url) throw Error('Each finding needs the source URL it came from');
...
export async function verifyFindings(findings, options = {}) {
  ...
  for (const finding of findings) {
    if (!pages.has(finding.url)) pages.set(finding.url, await fetchSource(finding.url, options));
    const page = pages.get(finding.url);
    if (!page.ok) { verified.push({ ...finding, verified: false, reason: `Source could not be re-fetched: ${page.error}`, fetchedAt: page.fetchedAt }); continue; }
    const present = normalise(page.excerpt).includes(normalise(finding.quote));
    verified.push({
      ...finding, verified: present, fetchedAt: page.fetchedAt, title: page.title,
      reason: present ? 'Quote found in the re-fetched page' : (page.truncated
        ? 'Quote not found in the retrieved portion of the page; it may lie beyond the fetched excerpt'
        : 'Quote not found in the re-fetched page')
    });
  }
  return verified;
}

// Findings become citable evidence only where the quote was actually confirmed.
export function evidenceFromFindings(question, provider, findings) {
  const confirmed = findings.filter(f => f.verified);
  const sources = confirmed.map(f => ({ url: f.url, title: f.title || null, fetchedAt: f.fetchedAt, ok: true, excerpt: `${f.claim}\n\nVerbatim quote: "${f.quote}"`, truncated: false }));
  return {
    question, at: new Date().toISOString(), searchedBy: provider, findings, sources,
    retrieved: confirmed.length,
    failed: findings.filter(f => !f.verified).map(f => ({ url: f.url, error: f.reason })),
    review: null,
    note: `${provider} searched the web; the coordinator re-fetched every cited page and kept only the ${confirmed.length} of ${findings.length} findings whose quote was confirmed. Confirming a quote proves the source says it, not that it is true or current.`
  };
}
