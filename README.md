...
On Windows, double-click **`START HERE.cmd`**. That is the normal start button—no terminal commands or installation step. You can also run `start.cmd` or `npm start` from this folder. Both use `supervisor.mjs`, which restarts unexpected coordinator exits with bounded backoff and reloads durable state. Use `npm run start:once` only when you explicitly do not want crash supervision. It opens **http://127.0.0.1:4317/** in your browser by itself; set `COORDINATOR_NO_OPEN=1` if you would rather it did not, and the address is printed either way. It starts paused. `Ctrl+C` pauses dispatch, drains active bounded calls and shuts down. Do not run two servers against the same data directory.
...
Use **Pause after this step** to let active work drain, or **Stop now** to cancel active agent/test processes while preserving the project and queue. **Run** continues eligible queued work. **Restart coordinator** reloads durable state, and **Exit completely** shuts down the local server. Checkpoints contain task ownership, requirements, acceptance criteria, decisions, commits, tests and concise handoffs.
...
- **Failing tests start a repair round too**, with the failing commands, exit codes and output handed back to the builder. The coordinator runs the commands, so that output is the only way a builder can learn what its last attempt broke. Bounded by the same `maxRepairRounds`.
...
- On Windows, Codex automatically uses its worktree-bounded workspace sandbox because its read-only OS sandbox can reject repository reads. The coordinator independently validates proposed files, owned paths and Git history. Other platforms retain the narrow read-only default. Claude uses subscription login, plan mode and only Read/Glob/Grep tools. It has no shell or delegation tools in this adapter.
...
- Quota/auth failures mark the provider unavailable. Eligible tasks can move to another provider with the existing worktree and a short checkpoint, within attempt limits. When no independent provider remains, work waits. Codex subscription limits are read every five minutes through its app-server without model turns. Measured exhaustion blocks dispatch; measured recovery requeues waiting work without overriding Pause or enabling disabled providers. Kimi/Claude quota recovery uses small probes after 30/60/120 minutes and then every four hours while enabled work remains; calls count toward the shared budget. Their remaining percentages are unavailable. Authentication failures require manual attention. The dashboard server must remain open.
...
Continuous planning is on by default for the one-click workflow and can be turned off while paused.
...
**Discussion.** Agents relay rather than converse by default, so *Team discussion* gives every available agent one turn, in order, each seeing what the others have already said and asked to engage with it directly. One call per agent, no repeated rounds. Talking changes nothing on its own: record the outcome as a decision and every later plan, build and review carries it.

**Project memory.** Each integrated milestone is recorded from work that already happened — the builder's handoff and the reviewer's verdict — and the recent entries reach every later agent as `completedMilestones`. This costs no model call and no call budget, so milestone eight knows what milestone two established. The store is capped and each entry truncated, so a long run cannot grow prompts without limit.
...
Project reads are non-interactive within the adapter's existing restrictions. Codex uses a never-prompt approval policy. On Windows its file access is automatically limited to workspace-write so repository inspection works; every proposal is still independently constrained and validated by the coordinator. Claude allows its project Read rule and Glob/Grep tools; shell, file modification and delegation tools remain unavailable. The coordinator applies validated proposals and runs configured tests itself. No login, OS access, or sandbox override is automatically granted.
